...
Uses the app's own settings (`DATABASE_URL`) and the shared declarative
`Base` so `alembic revision --autogenerate` can diff the ORM models against
the live schema. Runs migrations through the async engine since the app
uses `asyncpg`.
"""
